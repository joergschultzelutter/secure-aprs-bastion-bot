...
- Program execution (`--launch-as-subprocess` switch):
  - The programs to be executed can be started either synchronously or as a detached process.
    - Synchronous execution first executes the desired script and waits until it has finished. After script termination,  `secure-aprs-bastion-bot` sends an APRS confirmation to the user. This is the default behavior where the configuration program's command option [`--launch-as-subprocess`](/docs/configure.md#parameters) was not set. Ideally, only use this option for short runs. Confirmation of processing via APRS is only sent to the caller after the script has been completed.
    - Asynchronous processing first sends the APRS confirmation to the user and starts the desired program _as a separate process_.  `secure-aprs-bastion-bot` will _not_ wait for the program to finish executing. Such processing may be necessary, for example, when restarting the server on which `secure-aprs-bastion-bot` is installed. The processing of the script is initiated as a background process and can therefore also take place over a longer period of time.
