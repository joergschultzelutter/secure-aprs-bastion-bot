...
- A: Yes, to a certain extent (see the previous explanations on account configuration and preventing multiple use of valid TOTP codes). Due to the nature of unencrypted APRS messages, there is a theoretical possibility of misuse if an attacker a) obtains a valid and unused TOTP code, b) has knowledge of the `--command-code`s associated with this configuration/call sign, and at the same time c) mimicks the call sign of the original sender. However, such an attacker would only be able to transmit ONE single `--command-code` to `secure-aprs-bastion-bot`; no additional command sequences would be possible for this TOTP code.
