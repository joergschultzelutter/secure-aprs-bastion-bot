...
| `510 not extended` | The identified `--command-string` still contains placeholders after `core-aprs-client` [replaced the placeholders with the user's additional parameters](/README.md#anatomy-of-an-aprs-message-to-secure-aprs-bastion-bot). Usually, this means that you created a user script with placeholders - but the APRS user did not submit a lower number of additional parameters to `core-aprs-client`. | 
