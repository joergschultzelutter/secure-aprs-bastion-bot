...
| `--aprs-test-arguments`  | Used in combination with either `--test-totp-code` or `--execute-totp-code`. Simulates parameter input `$1`..`$9` from an incoming APRS message. 0..9 parameters are supported. Input Parameter `$0` always contains the user's callsign. | `str`  | `[]` (empty list)  |
...
| `DF1JSL`                    | `123456`             | :x:                | SSID-less callsign cannot access callsign with SSID                                                                                                                   |
