...
* [Configuration file](#configuration-file)
  * [Configuration file - excerpt](#configuration-file---excerpt)
  * [Configuration file - complete sample template](#configuration-file---complete-sample-template)
<!--te-->
...
#
# Number of maximum callsign/TOTP dupe cache entries
sabb_totp_cache_max_len = 100
#
# life span of a callsign/TOTP dupe cache in seconds
...
#
# Dry-run setting. Default = False (execute scripts)
# Set this value to "True" if secure-aprs-bastion-bot
# is supposed to simulate the execution of the --command-script.
sabb_dry_run = false
```
...
|-----------------------------------|--------|----------------------------|-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `sabb_command_config`             | `str`  | `sabb_command_config.yaml` | Name of the external configuration file (generated by [`configure.py`](configure.md)) which contains the approved users/callsigns and `--command-code`/`--command-script` configuration |
| `sabb_totp_cache_max_len`         | `int`  | `100`                      | Defines the maximum number of callsign/TOTP entries that are checked for ingress duplicates.                                                                                            |
| `sabb_totp_cache_max_age_seconds` | `int`  | `360` (6 mins)             | Sets the life span for a dupe detection's dictionary entry (unit of measure = seconds).                                                                                                 |
| `sabb_dry_run`                    | `bool` | `false`                    | When set to `true`, `secure-aprs-bastion-bot` will only simulate the execution of the `--command-script` value                                                                          |   


### Configuration file - complete sample template

This section displays the complete [preconfigured template file](/configuration_file_examples/secure_aprs_bastion_bot.cfg.TEMPLATE). All generic parts of the configuration file, such as the configuration of beacons and bulletins, are described in detail in the accompanying [configuration file documentation](https://github.com/joergschultzelutter/core-aprs-client/blob/master/docs/configuration.md).
...
aprsis_callsign = YOUR_APRS_CALLSIGN_GOES_HERE
#
...
aprsis_passcode = YOUR_APRSIS_PASSCODE_GOES_HERE
#
...
aprsis_server_filter = g/YOUR_APRS_CALLSIGN_GOES_HERE
